...
export default function PayZakatSilver({ village, csrf }: Props) {
    ...
    const [silverPrice, setSilverPrice] = useState<{ price_per_gram: number; nisab_value: number } | null>(null);
    useEffect(() => {
        fetch('/silver-price')
            .then((r) => r.json())
            .then(setSilverPrice)
            .catch(() => setSilverPrice(null));
    }, []);
    const totalValue = silverPrice?.price_per_gram && form.weight ? parseFloat(form.weight) * silverPrice.price_per_gram : 0;
    ...
    const handleCalculate = async (e: React.FormEvent) => {
        e.preventDefault();
        setError(null);
        const res = await fetch('/silver-zakat/calculate', {
            method: 'POST',
            headers: {
    ...
    const createPayment = async () => {
        const res = await fetch('/payments', {
            ...
            body: JSON.stringify({
                ...
                description: `Zakat Perak ${new Date().toLocaleDateString('id-ID', {
                    month: 'long',
                    year: 'numeric',
                })}`,
            }),
    ...
    const handlePay = async () => {
        ...
        try {
            ...
            const res = await fetch('/silver-zakat', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'X-CSRF-TOKEN': csrf,
        ...
        <form onSubmit={handleCalculate} className="space-y-3">
            <div className="mt-4 text-center">
                ...
                <p className="text-sm text-gray-600">
                    ...
                    <strong>Rp {silverPrice?.price_per_gram ? silverPrice.price_per_gram.toLocaleString('id-ID') : '...'} / gram</strong>
                </p>
            </div>
            <input
                className="input"
                placeholder="Berat Perak (gram)"
                type="number"
                value={form.weight}
            ...
            {silverPrice && (
                <div className="text-sm text-gray-600">
                    Nisab zakat perak saat ini:
                    <strong className="ml-1">Rp {silverPrice.nisab_value.toLocaleString('id-ID')}</strong>
                </div>
            )}
            ...
            {result && (
                <div className="mt-4 text-center">
                    {/* Nilai perak user */}
                    <p className="text-sm text-gray-600">
                        Nilai perak Anda ({form.weight || 0}&nbsp;gram): <strong>Rp {totalValue.toLocaleString('id-ID')}</strong>
                    </p>
